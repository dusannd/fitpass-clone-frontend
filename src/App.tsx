...
/*
  LAZY ROUTES: these four pull in the two heaviest dependencies in the app -
  recharts (~400 kB) and html5-qrcode (~200 kB). Loaded eagerly, a member who
  never opens the scanner still downloaded and parsed the whole scanner, and an
  admin who never logged a workout still downloaded the charting library.

  TrainerClients is on this list for a reason that is easy to miss: it does not
  import recharts itself, it renders <ProgressCard />, which does. Leaving it
  eager would have pinned recharts into the main bundle and made splitting the
  other two pages worth almost nothing.
*/
const Workouts = lazy(() => import("./pages/member/Workouts.tsx"));
const TrainerClients = lazy(() => import("./pages/trainer/TrainerClients.tsx"));
const AdminAnalytics = lazy(() => import("./pages/admin/AdminAnalytics.tsx"));
const WorkerScanner = lazy(() => import("./pages/worker/WorkerScanner"));

export default function App() {
    return (
        /*
          Outer safety net. The lazy routes above all sit inside <Layout />, which
          has its own closer boundary so the sidebar survives the wait - React uses
          the nearest one. This is here for anything lazy added outside Layout later.
        */
